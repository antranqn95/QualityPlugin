...
import com.company.androidquality.tasks.CheckQualityTask
...
class AndroidQualityPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        ...
        val checkQuality = project.tasks.register("checkQuality", CheckQualityTask::class.java) {
            group = "Android Quality"
            description = "Run checkSecurity + detekt (if useDetekt=true) and generate quality-report.json"
            securityConfig = extension.security
            dependsOn(checkSecurity)
        }
        ...
        project.afterEvaluate {
            if (extension.security.useDetekt) {
                ...
                // Wire detekt into checkQuality: detekt → checkSecurity → checkQuality
                checkSecurity.configure { mustRunAfter("detekt") }
                checkQuality.configure { dependsOn("detekt") }
            }
