...
abstract class CheckSecurityTask : DefaultTask() {
    ...
    fun check() {
        ...
        if (securityConfig.generateReport) {
            ...
            reportDir.mkdirs()
            val reportFile = File(reportDir, "security-report.html")
            SecurityReportGenerator.generate(findings, reportFile, project.name)
            logger.lifecycle("[AndroidQuality] Security report: ${reportFile.relativeTo(project.projectDir)}")
        }
