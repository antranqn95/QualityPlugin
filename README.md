...
When `generateReport = true`, running `checkSecurity` **automatically generates the Markdown report** — no separate task needed.

```bash
# Single command: scan + report
./gradlew :app:checkSecurity

# Also merge Detekt findings (requires useDetekt = true)
./gradlew :app:detekt :app:checkSecurity
```

Or use the dedicated task (same result):
```bash
